...
(function( jQuery ) {
	...
	var methods = {
		...
		*/
		createTextEditor: function() {
			...
			jQuery(methods.el).hide();

			// Create a container which will hold or text editor
			methods.container = jQuery("<div />").addClass( methods.settings.containerClass ).css({
				"float"   : "left",
			...
			jQuery(methods.el).after( methods.container );

			// Create the iFrame and append to the previously created container
			methods.editor = jQuery("<iframe />").addClass( methods.settings.iFrameClass ).css({
				"float"   : "left",
				"width"   : methods.settings.width,
			...
			jQuery(methods.editor).contents().find("body").css({
			...
			jQuery(methods.editor).contents().find("body").append("<p> </p>");

			// Add some css to the iFrame
			var iFrameCSS = '<style type="text/css">body{padding:2%;}p{margin:0;}</style>';
			jQuery(methods.editor).contents().find("head").append(iFrameCSS);

			// Build the button container
			methods.buttons = jQuery("<div />").addClass( methods.settings.buttonsClass ).css({
				"float"   : "left",
				"width"   : methods.settings.width
		...
		*/
		setContent: function() {
			...
			var content = jQuery(methods.el).text();

			// Put the content of the textarea into the editor
			jQuery( methods.editor ).contents().find("body").append(content);
		},
		...
		*/
		createButtons: function() {
			...
			for( i = 0; i < defaultOptions.length; i++ ) {
		        ...
		        jQuery("<a />").addClass( button.command ).text( button.content ).data( "command", button.command ).appendTo( methods.buttons );
			}
		},
		...
		*/
		events: function() {
			...
			jQuery("." + methods.settings.buttonsClass + " a").on("click", function(e) {
			...
			jQuery( methods.editor ).contents().find("body").on("keydown", function(e) {
			...
			jQuery( methods.editor ).contents().find("body").on("keyup", function(e) {

				// Check or the text is changed
				var changed = ( jQuery( methods.editor ).contents().find("body").html() !== jQuery(methods.el).text() ) ? true : false;
			...
			jQuery( methods.el ).parents("form").on("submit", function(e) {
		...
		*/
		cleanTheCode: function() {
			...
			jQuery(methods.editor).contents().find("body").find("br").removeAttr("class").unwrap();
		...
		cleanLists: function( command ) {
    		...
    		// Remove the UL or OL
    		jQuery(methods.editor).contents().find("body").find(type).removeAttr("class").unwrap();
		},
		...
		*/
		putContentBack: function() {
			...
			var postData = jQuery(methods.editor).contents().find("body").html();

			// Make sure the textarea is empty
			jQuery( methods.el ).val( postData );
		},
		...
		*   textarea. Use this when the texteditor is
		*   not used in a form
		*
		*/
		...
		returnText: function() {
			methods.cleanTheCode();
			methods.cleanLists();
			var text = jQuery(methods.editor).contents().find("body").html();
	...
	jQuery.fn.texteditor = function( method ) {
		...
		return this.each( function() {
			...
			if ( methods[method] ) {
				...
				jQuery.error( 'Method ' +  method + ' does not exist on jQuery.texteditor' );
			}
		});
